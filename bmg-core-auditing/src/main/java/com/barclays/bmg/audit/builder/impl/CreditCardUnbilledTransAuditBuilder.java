...
import com.barclays.bmg.utils.MaskingUtil;

public class CreditCardUnbilledTransAuditBuilder extends AbstractTransactionAuditBuilder {

    /**
     * The Constant named "SCRNNAM_BP" is created.
     */
    private static final String SCRNNAM_BP = "VIEW_CREDIT_CARD_UNBILLED_TRANS";

    @Override
    public TransactionAuditDTO buildFromRequest(Object[] args, Object result) {

	CreditCardUnbilledTransOperationRequest request = (CreditCardUnbilledTransOperationRequest) args[0];
	...
	field8.setValue(AuditConstant.SER_TYP_CCD_UNBILLED_STATEMENT);
	screenData.addField(field8);

	FieldDataDTO field9 = new FieldDataDTO();
	field9.setFieldId(AuditConstant.TXNSUBTYP);
	field9.setValue(AuditConstant.SUB_SER_TYP_CCD_UNBILLED_STATEMENT);
	...
	if (request != null) {
	    ...
	    field7.setValue(MaskingUtil.getMaskedCreditCardNumber(request.getCardNo()));
	...
	CreditCardUnbilledTransOperationResponse response = (CreditCardUnbilledTransOperationResponse) result;
	ScreenDataDTO screenData = new ScreenDataDTO();
