...
public class CreditCardLinkAuditBuilder extends AbstractTransactionAuditBuilder {

	/**
	 * The Constant named "SCRNNAM_BP" is created.
	 */
	private static final String SCRNNAM_BP = "LINK_YOUR_CREDIT_CARD";

	@Override
	public TransactionAuditDTO buildFromRequest(Object[] args, Object result) {

		CreditCardLinkConfirmOperationRequest request = (CreditCardLinkConfirmOperationRequest) args[0];
		...
		if (request != null) {
			...
			if (context != null) {
				...
				if (request.getContext().getPpMap() != null) {
					field1.setValue(context.getPpMap().get(ProductProcessorTypeCode._BK));
				}
	    ...
	    field7.setValue(request.getCreditCardNumber());
	    screenData.addField(field7);*/

			FieldDataDTO field7 = new FieldDataDTO();
			field7.setFieldId(AuditConstant.ACCOUNT_NUMBER);
			field7.setValue(request.getPrimaryAccountNumber());
			screenData.addField(field7);


			FieldDataDTO field10 = new FieldDataDTO();
			field10.setFieldId(AuditConstant.MOBNO);
			field10.setValue(request.getMobileNumber());
	...
	public TransactionAuditDTO buildFromResponse(Object[] args, Object result) {
		CreditCardLinkConfirmOperationResponse response = (CreditCardLinkConfirmOperationResponse) result;
		TransactionAuditDTO transactionAuditDTO = new TransactionAuditDTO();
		ScreenDataDTO screenData = new ScreenDataDTO();
		...
		if (request != null) {
			...
			FieldDataDTO field10 = new FieldDataDTO();
			field10.setFieldId(AuditConstant.MOBNO);
			field8.setValue(request.getMobileNumber());
			screenData.addField(field10);
		}
