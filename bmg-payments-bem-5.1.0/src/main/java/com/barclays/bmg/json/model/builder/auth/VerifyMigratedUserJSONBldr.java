 ...
 * 
 */
public class VerifyMigratedUserJSONBldr implements BMBJSONBuilder {
	...
	if (responseContext instanceof VerifyMigratedUserOperationResponse) {
	    VerifyMigratedUserOperationResponse response = (VerifyMigratedUserOperationResponse) responseContext;
     ...
     */
    protected BMBPayloadHeader createHeader(VerifyMigratedUserOperationResponse response) {
     ...
     * @return void
     */
    protected void populatePayLoad(VerifyMigratedUserOperationResponse response, BMBPayload bmbPayload) {
	VerifyMigratedUserJSONResponseModel responseModel = null;
	if (response != null && response.isSuccess()) {
	    responseModel = new VerifyMigratedUserJSONResponseModel();
	    if (response != null && response.isSuccess()) {

		/*
		 * responseModel.setCryptoPinStatus(response.getCryptoPinStatus()); responseModel.setUserStatusInMCE(response.getUserStatusInMCE());
		 * responseModel.setLangPref(response.getLangPref()); responseModel.setScvId(response.getScvId());
		 */
	    }
	}
