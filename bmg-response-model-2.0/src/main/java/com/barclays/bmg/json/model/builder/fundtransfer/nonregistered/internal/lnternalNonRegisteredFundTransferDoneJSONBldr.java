...
public class lnternalNonRegisteredFundTransferDoneJSONBldr extends BMBMultipleResponseJSONBuilder implements BMBJSONBuilder{
	...
	protected BMBPayloadHeader createHeader(ResponseContext response) {
		...
		header.setResId(ResponseIdConstants.INTERNAL_NON_REGISTERED_FUND_TRANSFER_DETAILS_DONE);
	...
	protected void populatePayLoad(BMBPayload bmbPayload,
		...
		DomesticFundTransferExecuteOperationResponse response =
				(DomesticFundTransferExecuteOperationResponse)responses[0];
		...
		if(response !=null && response.isSuccess()){
			...
			paymentConfirmBean.setTxnRefNo(txnRefNo);
