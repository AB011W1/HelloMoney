 ...
 */
public class UpdateBeneficiaryJSONBldr extends BMBMultipleResponseJSONBuilder
	...
	protected void populatePayLoad(BMBPayload bmbPayload,
		...
		UpdateBeneficiaryOperationResponse updateBeneficiaryOperationResponse = (UpdateBeneficiaryOperationResponse) responses[0];

		BeneficiaryDTO beneficiaryDTO = updateBeneficiaryOperationResponse
				.getBeneficiaryDTO();

		AddBeneficiaryJSONModel beneficiaryJSONModel = new AddBeneficiaryJSONModel(
				beneficiaryDTO);
		Context context = updateBeneficiaryOperationResponse.getContext();
		responseModel.setBeneficiaryInfo(beneficiaryJSONModel);
		responseModel.setTxnRefNo(context.getOrgRefNo());
		responseModel
				.setTxnDate(BMGFormatUtility.getLongDate(DateTimeUtil
						.getCurrentBusinessCalendar(
								updateBeneficiaryOperationResponse.getContext())
						.getTime()));
