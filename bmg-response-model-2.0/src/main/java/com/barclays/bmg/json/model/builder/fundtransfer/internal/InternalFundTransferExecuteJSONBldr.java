...
public class InternalFundTransferExecuteJSONBldr extends BMBMultipleResponseJSONBuilder{
	...
	public Object createMultipleJSONResponse(
		...
		for(ResponseContext response: responseContexts){
			if(response!=null && !response.isSuccess()){
				bmbPayloadHeader = createHeader(response, ResponseIdConstants.INTERNAL_FUND_TRANSFER_EXECUTE_RESPONSE_ID);
				break;
			}
		}
		...
		if(bmbPayloadHeader!=null){
			...
			bmbPayload = new BMBPayload(createHeader(responseContexts[0],ResponseIdConstants.INTERNAL_FUND_TRANSFER_EXECUTE_RESPONSE_ID));
