...
public class ExternalFundTransferInitJSONBldr extends BMBMultipleResponseJSONBuilder {
	...
	protected BMBPayloadHeader createHeader(ResponseContext response) {
		...
		header.setResId(ResponseIdConstants.External_Fund_Transfer_Payee_List);
		return header;
	...
	protected void populatePayLoad(BMBPayload bmbPayload,
		...
		List<CategorizedPayeeListDTO> categorizedPayeeList = payeeListOperationResponse.getCategorizedPayeeList();
		...
		for(CategorizedPayeeListDTO categorizedPayee: categorizedPayeeList){
			...
			if(payeeList!=null){
				for(BeneficiaryDTO payee : payeeList){
					...
					beneficiary.setActNo(payee.getDestinationAccountNumber());
	...
	@Override
	public Object createJSONResponse( ResponseContext responseContexts) {
		BMBPayloadHeader bmbPayloadHeader = null;
		BMBPayload bmbPayload = null;
			if(responseContexts!=null && !responseContexts.isSuccess()){
				bmbPayloadHeader = createHeader(responseContexts);
			}

		if(bmbPayloadHeader!=null){
			bmbPayload = new BMBPayload(bmbPayloadHeader);
		}else{
			bmbPayload = new BMBPayload(createHeader(responseContexts));
			populatePayLoadAC(bmbPayload, responseContexts);
		}

		return bmbPayload;
	}

	protected void populatePayLoadAC(BMBPayload bmbPayload,
			ResponseContext responses) {
		ExternalFundTransferInitJSONModel responseModel = new ExternalFundTransferInitJSONModel();
		RetrieveAcctListOperationResponse acctListOperationResponse = (RetrieveAcctListOperationResponse)responses;
		responseModel.setSrcLst(getCASAAccountList(acctListOperationResponse.getAcctList(),acctListOperationResponse));
		bmbPayload.setPayData(responseModel);
	}
