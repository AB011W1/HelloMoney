...
public class StructuredNoteDetailsPayloadAdapter {
	...
	StructuredNoteDetailsServiceRequest strutNoteDetailsServiceRequest = (StructuredNoteDetailsServiceRequest) args[0];

	IndividualCustomerInfo individualCustomerInfo = new IndividualCustomerInfo();

	if (individualCustomerInfo != null) {

	    individualCustomerInfo.setAssetTypeCode(strutNoteDetailsServiceRequest.getAssetClass());

	    individualCustomerInfo.setCustomerNumber(strutNoteDetailsServiceRequest.getContext().getPpMap().get(ProductProcessorTypeCode._IN));
