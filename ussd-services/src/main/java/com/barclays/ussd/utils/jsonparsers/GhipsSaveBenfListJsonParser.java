...
public class GhipsSaveBenfListJsonParser implements BmgBaseJsonParser {

	/** The Constant LOGGER. */
	private static final Logger LOGGER = Logger
			.getLogger(GhipsSaveBenfListJsonParser.class);
	...
	public MenuItemDTO parseJsonIntoJava(
		...
		try {
			PayeeList payeeList = mapper.readValue(responseBuilderParamsDTO
					.getJsonString(), PayeeList.class);
			if (payeeList != null) {
				if (payeeList.getPayHdr() != null
						&& USSDExceptions.SUCCESS.getBmgCode()
								.equalsIgnoreCase(
										payeeList.getPayHdr().getResCde())) {
					Collections.sort(payeeList.getPayData().getSrcLst(),
							new GhipsPayeeListCustomerAccountComparator());
					...
					throw new USSDNonBlockingException(payeeList.getPayHdr()
							.getResCde());
				} else {
					LOGGER.error("Error while servicing "
							+ responseBuilderParamsDTO.getBmgOpCode());
	 ...
	 * @param payeeList
	 */
	private void setOutputInSession(
			ResponseBuilderParamsDTO responseBuilderParamsDTO,
			PayeeList payeeList) {
		...
		if (payeeLstData != null) {
			...
			responseBuilderParamsDTO.getUssdSessionMgmt().setTxSessions(
					txSessionMap);
			if (payeeLstData.getSrcLst() != null
					&& !payeeLstData.getSrcLst().isEmpty()) {
				responseBuilderParamsDTO.getUssdSessionMgmt().getTxSessions()
						.put(
								USSDInputParamsEnum.GHIPS_OTHER_BANK_FT_AMT
										.getTranId(), payeeLstData.getSrcLst());
			}
			List<ToList> lstFTto = payeeLstData.getPayLst();
			if (lstFTto != null && !lstFTto.isEmpty()) {
				ToList toList = lstFTto.get(0);
				if (toList != null && toList.getBnfLst() != null
						&& !toList.getBnfLst().isEmpty()) {
					responseBuilderParamsDTO.getUssdSessionMgmt()
							.getTxSessions().put(
									USSDInputParamsEnum.GHIPS_OTHER_BANK_FT_TO_AC
											.getTranId(), toList.getBnfLst());
	...
	public void setNextScreenSequenceNumber(MenuItemDTO menuItemDTO) {
		menuItemDTO
				.setNextScreenSequenceNumber(USSDSequenceNumberEnum.SEQUENCE_NUMBER_TWO
						.getSequenceNo());
	}
	...
	public int getCustomNextScreen(String userInput,
		...
		List<BeneData> payeeList = (List<BeneData>) ussdSessionMgmt
				.getTxSessions().get(
						USSDInputParamsEnum.GHIPS_OTHER_BANK_FT_TO_AC.getTranId());
		if (payeeList != null && payeeList.size() == 1) {
			userInputMap = userInputMap == null ? new HashMap<String, String>()
					: userInputMap;
			userInputMap.put(USSDInputParamsEnum.GHIPS_OTHER_BANK_FT_TO_AC
					.getParamName(), USSDConstants.DEFAULT_OPTION_SELECTION);
			ussdSessionMgmt.getUserTransactionDetails().setUserInputMap(
					userInputMap);
			seqNo = USSDSequenceNumberEnum.SEQUENCE_NUMBER_THREE
					.getSequenceNo();
		}
		return seqNo;
	}
	...
	class GhipsPayeeListCustomerAccountComparator implements Comparator<AccountDetails>, Serializable {
