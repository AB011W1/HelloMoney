...
/**
 * @author BTCI This class is used for Casa Detail Parsing. Change MainAcctMiniStmtJSONParser also in case of any structural or functional changes.
 * 
 */
public class CasaDetailJSONParser implements BmgBaseJsonParser {

    private static final String DEFAULT_MINI_STMT_CNT = "10";
    private static final Logger LOGGER = Logger.getLogger(CasaDetailJSONParser.class);
	...
	if (acntDet != null) {
		...
		if (ele.getCrAmt() != null && !StringUtils.isEmpty(ele.getCrAmt().getAmt())) {
		    pageBody.append(ele.getCrAmt().getAmt());
		    pageBody.append(USSDConstants.SINGLE_WHITE_SPACE).append(acntDet.getAvblBal().getCurr());
		} else if (ele.getDrAmt() != null && !StringUtils.isEmpty(ele.getDrAmt().getAmt())) {
		    pageBody.append(ele.getDrAmt().getAmt());
		    pageBody.append(USSDConstants.SINGLE_WHITE_SPACE).append(acntDet.getAvblBal().getCurr());
		}
	...
	String actNo = ussdSessionMgmt.getUserTransactionDetails().getUserInputMap().get(USSDInputParamsEnum.MINI_STMT_SEL_AC.getParamName());
	CustomerMobileRegAcct acctDet = lstAccntDet.get(Integer.parseInt(actNo) - 1);
	return acctDet;
    }
