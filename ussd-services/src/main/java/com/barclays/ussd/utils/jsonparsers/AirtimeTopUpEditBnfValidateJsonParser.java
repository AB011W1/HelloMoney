...
public class AirtimeTopUpEditBnfValidateJsonParser implements BmgBaseJsonParser {


    private static final String AIRTIME_TOPUP_SAV_BNF_CONFIRM_MSG = "airtimetopup.edit.bnf.confirm.msg";
    private static final String CONFIRM_LABEL = "label.confirm";
    private static final Logger LOGGER = Logger.getLogger(AirtimeTopUpEditBnfValidateJsonParser.class);
	...
	String mobilenumber = userInputMap.get(USSDInputParamsEnum.AIRTIME_MOB_NUM.getParamName());
		...
		List<Biller> mnoList = (List<Biller>) txSessions.get(USSDInputParamsEnum.AIRTIME_TOPUP_NEW_BENE_SRC_MON.getTranId());
		...
		String mnoSelected =biller.getBillerName();
		 ...
		 String beneficiarySelected = userInputMap.get(USSDInputParamsEnum.AIRTIME_TOPUP_PAYEE_LIST.getParamName());
		 String nickName="";
		  if (!lstBenef.isEmpty()) {
    ...
    responseBuilderParamsDTO.getUssdSessionMgmt().getTxSessions().put(USSDInputParamsEnum.AIRTIME_TOPUP_EDIT_BEN_VALIDATE.getTranId(), txnRefNo);
