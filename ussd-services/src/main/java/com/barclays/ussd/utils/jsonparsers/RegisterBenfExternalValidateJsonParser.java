...
public class RegisterBenfExternalValidateJsonParser implements BmgBaseJsonParser,ScreenSequenceCustomizer {
    ...
    private static final String NIBNO_LABEL = "lable.nib";
    private static final String LABEL_NAME = "lable.name";
    private static final String CONFIRM_LABEL = "label.confirm";
    private static final String CONFIRM_LABEL_KE = "label.reg.confirm";
    private static final String LABEL_CONFIRM_HEADER_MESSAGE ="label.confirm.header.message";
	...
	try {
		...
		if (regBenfInt.getPayHdr() != null && USSDExceptions.SUCCESS.getBmgCode().equalsIgnoreCase(regBenfInt.getPayHdr().getResCde())) {
		    ...
		    if(transNodeId.equals("ussd0.4.3.4.2"))
		    	responseBuilderParamsDTO.getUssdSessionMgmt().getTxSessions().put(USSDInputParamsEnum.REG_BENF_BENF_CONFIRM.getTranId(), txnRefNo);
		    else
	...
	if(responseBuilderParamsDTO.getUssdSessionMgmt().getBusinessId().equals("MZBRB")){
	    ...
	    pageBody.append(nibNoLabel);
	    pageBody.append(userInputMap.get(USSDInputParamsEnum.REG_BENF_GET_NIB_NO.getParamName()));
	    //pageBody.append(USSDConstants.NEW_LINE);
	}else{
    if(transNodeId.equals("ussd0.4.3.4.2")){
	...
	if (validateRegBenfIntPayData != null && validateRegBenfIntPayData.getBeneficiaryInfo() != null) {
	  ...
	  /*  if(transNodeId.equals("ussd0.4.3.4.2")){
		...
		if (!USSDInputParamsEnum.REG_BEN_INT_VALIDATE.getTranId().equalsIgnoreCase(responseBuilderParamsDTO.getTranDataId())&&!transNodeId.equals("ussd0.4.3.4.2")) {
