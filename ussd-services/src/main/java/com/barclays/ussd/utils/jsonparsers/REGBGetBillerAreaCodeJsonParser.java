...
public class REGBGetBillerAreaCodeJsonParser implements BmgBaseJsonParser {
    ObjectMapper mapper = new ObjectMapper();

    public MenuItemDTO parseJsonIntoJava(ResponseBuilderParamsDTO responseBuilderParamsDTO) throws USSDNonBlockingException {
	USSDSessionManagement ussdSessionMgmt = responseBuilderParamsDTO.getUssdSessionMgmt();
	populateTxSession(responseBuilderParamsDTO);
	List<BillerArea> billerAreaList = (List<BillerArea>) ussdSessionMgmt.getTxSessions()
		.get(USSDInputParamsEnum.REG_BILLER_AREA_CODE.getTranId());
	...
	if (StringUtils.equalsIgnoreCase(USSDConstants.BUSINESS_ID_UGBRB, businessId)) {
	    ...
	    BillersListDO billersListDO = blrsLstDO
		    .get(Integer.parseInt(userInputMap.get(USSDInputParamsEnum.REG_BILLER_GET_BILLERS.getParamName())) - 1);
	    if (StringUtils.equalsIgnoreCase(billersListDO.getBillerId(), "NWSC-4")) {
		List<BillerArea> billerArea = extractBillerAttr(billersListDO.getBillerAttributes(), userInputMap);
		ussdSessionMgmt.getTxSessions().put(USSDInputParamsEnum.REG_BILLER_AREA_CODE.getTranId(), billerArea);
	    }
	}
    }
	...
	//    String searchChars = userInputMap.get(USSDInputParamsEnum.REG_BILLER_AREA_SEARCHER.getParamName());
	    List<BillerArea> areaList = billerAttributes.getAreaList();
	    /*for (BillerArea area : areaList) {
		if (StringUtils.startsWithIgnoreCase(area.getAreaName(), searchChars)) {
		    searchedBillerArea.add(area);
		}
	    }
	    if (searchedBillerArea.isEmpty()) {
