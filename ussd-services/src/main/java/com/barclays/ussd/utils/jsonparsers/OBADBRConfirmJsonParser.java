...
public class OBADBRConfirmJsonParser implements BmgBaseJsonParser {
    private static final String DELETIONSUCCESS_LABEL = "label.benefdeleted.success";
    private static final String DELETIONSUCCESS_LABEL_MZ = "label.delete.success";

    /** The Constant LOGGER. */
    private static final Logger LOGGER = Logger.getLogger(OBADBRConfirmJsonParser.class);
    ...
    public MenuItemDTO parseJsonIntoJava(ResponseBuilderParamsDTO responseBuilderParamsDTO) throws USSDNonBlockingException {
    	...
    	if (delBenefConfirm != null) {
    	    ...
    	    if(responseBuilderParamsDTO.getUssdSessionMgmt().getBusinessId().equals("MZBRB"))
    	    	successMessage = responseBuilderParamsDTO.getUssdResourceBundle().getLabel(DELETIONSUCCESS_LABEL_MZ,
    	    		    new Locale(ussdSessionMgmt.getUserProfile().getLanguage(), ussdSessionMgmt.getUserProfile().getCountryCode()));
    	    else
    	    	successMessage = responseBuilderParamsDTO.getUssdResourceBundle().getLabel(DELETIONSUCCESS_LABEL,
    	    			new Locale(ussdSessionMgmt.getUserProfile().getLanguage(), ussdSessionMgmt.getUserProfile().getCountryCode()));
