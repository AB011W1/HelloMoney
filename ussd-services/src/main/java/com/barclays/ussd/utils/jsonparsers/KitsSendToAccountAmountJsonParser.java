...
public class KitsSendToAccountAmountJsonParser implements BmgBaseJsonParser, SystemPreferenceValidator{

    @Autowired
    private ListValueResServiceImpl listValueResService;

    private static final Logger LOGGER = Logger.getLogger(KitsSendToAccountAmountJsonParser.class);
	...
	String sysPrefKitsMaxAmt=null;
	    ...
	    sysPrefKitsMinAmt = getSystemPreferenceData(userProfile, SystemPreferenceConstants.SYS_PARAM_KITS_PTA,
		SystemPreferenceConstants.MIN_KITS_PTA_AMT);
	    sysPrefKitsMaxAmt = getSystemPreferenceData(userProfile, SystemPreferenceConstants.SYS_PARAM_KITS_PTA,
		SystemPreferenceConstants.MAX_KITS_PTA_AMT);
 ...
 // End Fix Fix Fix
	...
	List<AccountDetails> accList= (List<AccountDetails>) ussdSessionMgmt.getTxSessions().get(USSDInputParamsEnum.KITS_STA_ACCOUNT_NUM.getTranId());
