...
public class MobileWalletTopupCreditCardORCasaJsonParser implements
		BmgBaseJsonParser, ScreenSequenceCustomizer {
	...
	private MenuItemDTO renderMenuOnScreen(
		...
		String TRANSACTION_AIRTIME_CASA_ACCOUNT = responseBuilderParamsDTO
				.getUssdResourceBundle().getLabel("label.casaaccounts", locale);
		String TRANSACTION_AIRTIME_CREDIT_CARD = responseBuilderParamsDTO
				.getUssdResourceBundle().getLabel("label.creditcards", locale);
		String TRANSACTION_AIRTIME_LABEL = responseBuilderParamsDTO
				.getUssdResourceBundle().getLabel("label.airtime.select.casacredit", locale);
		...
		pageBody.append(USSDConstants.DOT_SEPERATOR).append(
				TRANSACTION_AIRTIME_CASA_ACCOUNT);
		pageBody.append(USSDConstants.NEW_LINE);
		pageBody.append(accountIndex++);
		pageBody.append(USSDConstants.DOT_SEPERATOR).append(
				TRANSACTION_AIRTIME_CREDIT_CARD);
		pageBody.append(USSDConstants.NEW_LINE);

		// CR82

		/*responseBuilderParamsDTO.getUssdSessionMgmt()
			.getUserTransactionDetails().getCurrentRunningTransaction().setHomeOptionReq("TRUE");*/
	...
	public int getCustomNextScreen(String userInput,
		...
		if (userInput.equalsIgnoreCase("2")) {
			seqNo = USSDSequenceNumberEnum.SEQUENCE_NUMBER_TWENTYTHREE
					.getSequenceNo();
		}
		return seqNo;
	}
