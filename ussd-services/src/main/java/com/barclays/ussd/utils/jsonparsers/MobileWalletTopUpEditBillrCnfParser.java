...
public class MobileWalletTopUpEditBillrCnfParser implements BmgBaseJsonParser {
	  private static final String AIRTIME_SAVE_BNF_SUCCESS_LABEL = "label.airtime.editbenf.success";
	    private static final Logger LOGGER = Logger.getLogger(MobileWalletTopUpEditBillrCnfParser.class);
		...
		if (validateRegBillerPayData != null) {
			...
			if (responseBuilderParamsDTO.getUssdSessionMgmt().getTxSessions() != null
				&& responseBuilderParamsDTO.getUssdSessionMgmt().getTxSessions().get(USSDInputParamsEnum.MOBILE_WALLET_PAYEE_LIST.getTranId()) != null) {
