...
public class AirtimeTopUpGetBillerListJsonParser implements BmgBaseJsonParser {
    ...
    /** The Constant LOGGER. */
    private static final Logger LOGGER = Logger.getLogger(AirtimeTopUpGetBillerListJsonParser.class);
	...
	try {
		...
		if (billerLst.getPayHdr() != null && USSDExceptions.SUCCESS.getBmgCode().equalsIgnoreCase(billerLst.getPayHdr().getResCde())) {
			...
			if (USSDConstants.BILL_PAYMENT_PAY_CATEGORY.equalsIgnoreCase(ele.getPayCat())) {
				...
				throw new USSDNonBlockingException(USSDExceptions.BPY00660_AIRTIME_EMPTY.getBmgCode());
			    } else if (maxBillers <= benfSize) {
				ussdSessionMgmt.getTxSessions().put(USSDInputParamsEnum.AIRTIME_TOPUP_PAYEE_LIST.getTranId(),
					ele.getBnfLst().subList(0, maxBillers));
			    } else if (maxBillers >= benfSize) {
				ussdSessionMgmt.getTxSessions().put(USSDInputParamsEnum.AIRTIME_TOPUP_PAYEE_LIST.getTranId(), ele.getBnfLst());
			    }
