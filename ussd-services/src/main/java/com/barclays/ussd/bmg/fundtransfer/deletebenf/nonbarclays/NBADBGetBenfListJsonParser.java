...
public class NBADBGetBenfListJsonParser implements BmgBaseJsonParser
// , ScreenSequenceCustomizer
{
	...
	try {
		...
		if (accList.getPayHdr() != null && USSDExceptions.SUCCESS.getBmgCode().equalsIgnoreCase(accList.getPayHdr().getResCde())) {
		    ...
		    responseBuilderParamsDTO.getUssdSessionMgmt().getTxSessions()
			    .put(USSDInputParamsEnum.NON_BARC_DEL_BENF_PAYEE.getTranId(), bnfLst);
	...
	if (benfList != null && benfList.size() == 1) {
	    ...
	    userInputMap.put(USSDInputParamsEnum.NON_BARC_DEL_BENF_PAYEE.getParamName(), USSDConstants.DEFAULT_OPTION_SELECTION);
