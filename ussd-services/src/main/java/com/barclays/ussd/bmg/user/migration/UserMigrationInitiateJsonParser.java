...
public class UserMigrationInitiateJsonParser implements BmgBaseJsonParser, ScreenSequenceCustomizer {
    private static final Logger LOGGER = Logger.getLogger(UserMigrationInitiateJsonParser.class);
	...
	try {
		...
		if (USSDExceptions.SUCCESS.getBmgCode().equalsIgnoreCase(payHdr.getResCde())) {
		    // menuDTO = null;
		} else if (BEM_ERRCODE_USERREGEXISTS.equalsIgnoreCase(payHdr.getResCde()) || BEM_ERRCODE_08776.equalsIgnoreCase(payHdr.getResCde())
			|| BEM_ERRCODE_08778.equalsIgnoreCase(payHdr.getResCde())) {
		    LOGGER.error("User is already registered " + responseBuilderParamsDTO.getBmgOpCode());
		    // menuDTO = null;
	...
	if (StringUtils.equalsIgnoreCase(businessId, USSDConstants.BUSINESS_ID_KEBRB)) {
