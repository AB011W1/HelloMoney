...
public class FtGWTranStatusSubmitJsonParser implements BmgBaseJsonParser {

	private static final Logger LOGGER = Logger.getLogger(FtGWTranStatusSubmitJsonParser.class);
	...
	throws USSDBlockingException, USSDNonBlockingException {
		...
		try {
			IntNRFundTxConfirm otBnkFTExec = mapper.readValue(responseBuilderParamsDTO.getJsonString(), IntNRFundTxConfirm.class);
			 if (otBnkFTExec != null) {
					if (otBnkFTExec.getPayHdr() != null && USSDExceptions.SUCCESS.getBmgCode().equalsIgnoreCase(otBnkFTExec.getPayHdr().getResCde())) {
					    menuDTO = renderMenuOnScreen(responseBuilderParamsDTO, otBnkFTExec.getPayData().getRefNo());
					} else if (isInProgressErrorCode(otBnkFTExec.getPayHdr().getResCde())) {
						menuDTO = renderMenuOnScreen(responseBuilderParamsDTO, otBnkFTExec.getPayHdr().getTxnRefNo());
	...
	private MenuItemDTO renderMenuOnScreen(ResponseBuilderParamsDTO responseBuilderParamsDTO, String oTBnkFTExecData) {
		MenuItemDTO menuItemDTO = null;
		StringBuilder pageBody = new StringBuilder();
		if (oTBnkFTExecData != null) {
		menuItemDTO=new MenuItemDTO();
		String payReqAccepted= getLabel(responseBuilderParamsDTO, USSDConstants.LABEL_TRANSACTION_SUCCESS);
		String payReqRejected= getLabel(responseBuilderParamsDTO, USSDConstants.LABEL_TRANSACTION_FAILED);
		String mobileLabel= getLabel(responseBuilderParamsDTO, USSDConstants.USSD_TRANSACTION_MWALLETE_MOBILE);
		String trxnId= getLabel(responseBuilderParamsDTO, USSDConstants.TRAN_ID_LABEL_ID);
		int status=Integer.parseInt(responseBuilderParamsDTO.getUssdSessionMgmt().getUserTransactionDetails().getUserInputMap().get("status"));
		if(status==1)
			pageBody.append(payReqAccepted);
		else
			pageBody.append(payReqRejected);
		pageBody.append(USSDConstants.NEW_LINE);
		pageBody.append(trxnId);
		pageBody.append(oTBnkFTExecData);
		pageBody.append(USSDConstants.NEW_LINE);
		pageBody.append(mobileLabel);
		pageBody.append(responseBuilderParamsDTO.getUssdSessionMgmt().getMsisdnNumber());
