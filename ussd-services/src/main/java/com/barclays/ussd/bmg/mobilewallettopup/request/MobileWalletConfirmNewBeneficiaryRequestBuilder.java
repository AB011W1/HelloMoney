...
public class MobileWalletConfirmNewBeneficiaryRequestBuilder implements BmgBaseRequestBuilder {
	@Autowired
	...
	public USSDBaseRequest getRequestObject(RequestBuilderParamsDTO requestBuilderParamsDTO) {
		...
		.get(USSDInputParamsEnum.MOBILE_WALLET_NEW_BENE_SRC_MON.getTranId());

		String mnoId="",billerId = "";
		if(mnoList!=null && mnoList.size() != 0){
			billerId = mnoList.get(Integer.parseInt(userInputMap.get(USSDInputParamsEnum.MOBILE_WALLET_NEW_BENE_SRC_MON.getParamName())) - 1).getBillerId();//(mnoList.get(Integer.parseInt(userInputMap.get(USSDInputParamsEnum.MOBILE_WALLET_MNOS_LST.getParamName())) - 1)).getBillerName().toString();
		...
		//mnoId=mobileWalletConfirmBeneficiaryResponseParser.getMnoId(ussdSessionMgmt.getBusinessId(), functionality);
