...
public class BranchLocatorAreaNameListJsonParser implements BmgBaseJsonParser {
	...
	try {
	    ...
	    String areaLetter = userInputMap.get(USSDInputParamsEnum.BRANCH_LOCATOR_AREA_NAME.getParamName());
	    ...
	    List<UssdBranchLocatorLookUpDTO> cityNameList = (List<UssdBranchLocatorLookUpDTO>) ussdSessionMgmt.getTxSessions().get(
		    USSDInputParamsEnum.BRANCH_LOCATOR_CITY_LIST.getTranId());

	    UssdBranchLocatorLookUpDTO selectedCityName = cityNameList.get(Integer.parseInt(userInputMap
		    .get(USSDInputParamsEnum.BRANCH_LOCATOR_CITY_LIST.getParamName())) - 1);

	    areaList = ussdBranchLocatorLookUpDAOImpl.getAreaList(responseBuilderParamsDTO.getUssdSessionMgmt().getBusinessId(), selectedCityName
		...
		txSessions.put(USSDInputParamsEnum.BRANCH_LOCATOR_AREA_LIST.getTranId(), areaList);
