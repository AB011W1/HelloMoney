 ...
 * 
 */
public class EditBenfExtRetrieveBenfListJsonParser implements BmgBaseJsonParser {

    /** The Constant LOGGER. */
    private static final Logger LOGGER = Logger.getLogger(EditBenfExtRetrieveBenfListJsonParser.class);
	...
	if (payeeLstData != null) {
		...
		if (toList != null && toList.getBnfLst() != null && !toList.getBnfLst().isEmpty()) {
		    responseBuilderParamsDTO.getUssdSessionMgmt().getTxSessions().put(USSDInputParamsEnum.EDIT_BENF_EXT_BENEF_LIST.getTranId(),
			    toList.getBnfLst());
