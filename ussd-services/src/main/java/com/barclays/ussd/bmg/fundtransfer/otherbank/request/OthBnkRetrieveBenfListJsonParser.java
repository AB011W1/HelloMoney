...
import com.barclays.ussd.utils.jsonparsers.EBFTPayeeListJsonParser;
...
public class OthBnkRetrieveBenfListJsonParser implements BmgBaseJsonParser
// , ScreenSequenceCustomizer
{
	...
	try {
		...
		if (payeeList.getPayHdr() != null && USSDExceptions.SUCCESS.getBmgCode().equalsIgnoreCase(payeeList.getPayHdr().getResCde())) {
		    Collections.sort(payeeList.getPayData().getSrcLst(), new EBFTPayeeListCustomerAccountComparator());
	...
	if (payeeLstData != null) {
		...
		responseBuilderParamsDTO.getUssdSessionMgmt().getTxSessions()
			.put(USSDInputParamsEnum.EXT_BANK_FT_SEL_FRM_AC.getTranId(), payeeLstData.getSrcLst());
	    }
		...
		if (toList != null && toList.getBnfLst() != null && !toList.getBnfLst().isEmpty()) {
		    responseBuilderParamsDTO.getUssdSessionMgmt().getTxSessions()
			    .put(USSDInputParamsEnum.EXT_BANK_FT_TO_AC.getTranId(), toList.getBnfLst());
	...
	List<BeneData> payeeList = (List<BeneData>) ussdSessionMgmt.getTxSessions().get(USSDInputParamsEnum.EXT_BANK_FT_TO_AC.getTranId());
	if (payeeList != null && payeeList.size() == 1) {
	    userInputMap = userInputMap == null ? new HashMap<String, String>() : userInputMap;
	    userInputMap.put(USSDInputParamsEnum.EXT_BANK_FT_TO_AC.getParamName(), USSDConstants.DEFAULT_OPTION_SELECTION);
