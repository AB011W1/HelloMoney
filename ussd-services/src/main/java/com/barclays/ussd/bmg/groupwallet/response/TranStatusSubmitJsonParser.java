...
public class TranStatusSubmitJsonParser implements BmgBaseJsonParser {
	private static final Logger LOGGER = Logger.getLogger(TranStatusSubmitJsonParser.class);
			...
			RegBenfIntBean otBnkFTExec = mapper.readValue(responseBuilderParamsDTO.getJsonString(), RegBenfIntBean.class);
			 if (otBnkFTExec != null) {
					if (otBnkFTExec.getPayHdr() != null && USSDExceptions.SUCCESS.getBmgCode().equalsIgnoreCase(otBnkFTExec.getPayHdr().getResCde())) {
					    menuDTO = renderMenuOnScreen(responseBuilderParamsDTO, otBnkFTExec.getPayData().getTxnRefNo());
