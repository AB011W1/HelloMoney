 ...
 */
package com.barclays.ussd.bmg.deletebillers.request;
 ...
 * 
 */
public class DelBillersValidateRequest implements BmgBaseRequestBuilder {
	...
	String userInput = userInputMap.get(USSDInputParamsEnum.DEL_BILLERS_LIST.getParamName());
	String payeeId = "";
	if (requestBuilderParamsDTO.getUssdSessionMgmt().getTxSessions() != null
		&& requestBuilderParamsDTO.getUssdSessionMgmt().getTxSessions().get(USSDInputParamsEnum.DEL_BILLERS_LIST.getTranId()) != null) {
	    List<Beneficiery> lstBenef = (List<Beneficiery>) requestBuilderParamsDTO.getUssdSessionMgmt().getTxSessions().get(
		    USSDInputParamsEnum.DEL_BILLERS_LIST.getTranId());
	    if (!lstBenef.isEmpty()) {
	...
	requestParamMap.put(USSDInputParamsEnum.DEL_BILLER_DETAILS.getParamName(), payeeId);
