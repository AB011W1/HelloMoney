...
public class KitsSendToAccountConfirmDetailsRequestBuilder implements BmgBaseRequestBuilder{
	   /* @Autowired
		...
		List<AccountData> lstFromAcct = (List<AccountData>) requestBuilderParamsDTO.getUssdSessionMgmt().getTxSessions().get(
			USSDInputParamsEnum.KITS_STA_ACCOUNT_NUM.getTranId());
		...
		requestParamMap.put(USSDInputParamsEnum.KITS_STA_ACCOUNT_NUM.getParamName(), lstFromAcct.get(
			Integer.parseInt(userInputMap.get(USSDInputParamsEnum.KITS_STA_ACCOUNT_NUM.getParamName())) - 1).getActNo());
...
*/
	...
	String srcAcNoInput = ussdSessionMgmt.getUserTransactionDetails().getUserInputMap().get(
			USSDInputParamsEnum.KITS_STA_ACCOUNT_NUM.getParamName());
	...
	requestParamMap.put(USSDInputParamsEnum.KITS_STA_ACCOUNT_NUM.getParamName(), sourceAcct);
	requestParamMap.put(USSDInputParamsEnum.KITS_STA_BANK_CODE.getParamName(), branchCode);
	requestParamMap.put(USSDInputParamsEnum.KITS_STA_AMOUNT.getParamName(), userInputMap
			.get(USSDInputParamsEnum.KITS_STA_AMOUNT.getParamName()));
