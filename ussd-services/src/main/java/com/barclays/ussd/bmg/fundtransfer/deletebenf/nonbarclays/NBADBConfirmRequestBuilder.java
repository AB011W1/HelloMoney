...
public class NBADBConfirmRequestBuilder implements BmgBaseRequestBuilder {
    @SuppressWarnings("unchecked")
    public USSDBaseRequest getRequestObject(RequestBuilderParamsDTO requestBuilderParamsDTO) {
	USSDBaseRequest request = new USSDBaseRequest();

	Map<String, String> userInputMap = requestBuilderParamsDTO.getUssdSessionMgmt().getUserTransactionDetails().getUserInputMap();
	String payeeIdInput = userInputMap.get(USSDInputParamsEnum.NON_BARC_DEL_BENF_PAYEE.getParamName());
	List<OBAFTBeneficiary> bnfLst = (List<OBAFTBeneficiary>) requestBuilderParamsDTO.getUssdSessionMgmt().getTxSessions().get(
		USSDInputParamsEnum.NON_BARC_DEL_BENF_PAYEE.getTranId());
	String payeeId = bnfLst.get(Integer.parseInt(payeeIdInput) - 1).getPayId();
	userInputMap.clear();
	userInputMap.put(USSDInputParamsEnum.NON_BARC_DEL_BENF_VALIDATE.getParamName(), payeeId);
