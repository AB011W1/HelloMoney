...
public class BranchLocatorResponseJsonParser implements BmgBaseJsonParser {

    private static final String LABEL_BRANCH_ADDRESS = "label.branch.address";

    /** The Constant LOGGER. */
    private static final Logger LOGGER = Logger.getLogger(BranchLocatorResponseJsonParser.class);
	...
	if (branchAtmAddressLstData == null || branchAtmAddressLstData.getBrnLst() == null || branchAtmAddressLstData.getBrnLst().isEmpty()) {
	    throw new USSDNonBlockingException(USSDExceptions.USSD_NO_BRANCH_AVAILABLE.getBmgCode());
	}
	if (branchAtmAddressLstData.getBrnLst() != null) {
	    UssdResourceBundle ussdResourceBundle = responseBuilderParamsDTO.getUssdResourceBundle();
	    String addressLabel = ussdResourceBundle.getLabel(LABEL_BRANCH_ADDRESS, new Locale(ussdSessionMgmt.getUserProfile().getLanguage(),
		    ussdSessionMgmt.getUserProfile().getCountryCode()));
	    pageBody.append(addressLabel);
	    pageBody.append(USSDConstants.SINGLE_WHITE_SPACE);

	    int index = 1;
	    for (BranchAtmAddressLst branchAddress : branchAtmAddressLstData.getBrnLst()) {
		pageBody.append(USSDConstants.NEW_LINE);
		pageBody.append(index++);
		pageBody.append(USSDConstants.DOT_SEPERATOR);
		pageBody.append(branchAddress.getAddress());
	    }
