...
public class GhipsOneOffTransferAmtRequestBuilder implements BmgBaseRequestBuilder {
	...
	public USSDBaseRequest getRequestObject(
			...
			if(bankList!=null && bankList.size() != 0){
					...
					bankCode=ussdBranchLookUpDTO.getBankCode();
				requestParamMap.put(USSDInputParamsEnum.SELFREG_BRANCH.getParamName(), bankCode);
			}
			requestBuilderParamsDTO.getUssdSessionMgmt().getTxSessions().put(USSDInputParamsEnum.GHIPS_BANK_LIST.getParamName(),bankCode);
			requestParamMap.put(USSDInputParamsEnum.SELFREG_ACCOUNT.getParamName(), userInputMap.get(USSDInputParamsEnum.GHIPS_BANK_ACC_NO.getParamName()));
