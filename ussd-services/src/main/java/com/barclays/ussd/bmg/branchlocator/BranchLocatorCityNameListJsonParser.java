...
public class BranchLocatorCityNameListJsonParser implements BmgBaseJsonParser {

    /** The Constant LOGGER. */
    private static final Logger LOGGER = Logger.getLogger(BranchLocatorCityNameListJsonParser.class);
	...
	try {
	    ...
	    String cityLetter = userInputMap.get(USSDInputParamsEnum.BRANCH_LOCATOR_CITY_NAME.getParamName());
	    if (cityLetter.trim().equals("1")) {
		cityLetter = null;
	    }

	    cityList = ussdBranchLocatorLookUpDAOImpl.getCityList(responseBuilderParamsDTO.getUssdSessionMgmt().getBusinessId(), cityLetter);
		...
		txSessions.put(USSDInputParamsEnum.BRANCH_LOCATOR_CITY_LIST.getTranId(), cityList);
	    } else {
