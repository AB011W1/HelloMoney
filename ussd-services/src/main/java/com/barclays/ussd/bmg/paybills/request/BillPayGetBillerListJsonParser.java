...
public class BillPayGetBillerListJsonParser implements BmgBaseJsonParser {
    ...
    /** The Constant LOGGER. */
    private static final Logger LOGGER = Logger.getLogger(BillPayGetBillerListJsonParser.class);
	...
	try {
		...
		if (billerLst.getPayHdr() != null && USSDExceptions.SUCCESS.getBmgCode().equalsIgnoreCase(billerLst.getPayHdr().getResCde())) {
			...
			if (USSDConstants.BILL_PAYMENT_PAY_CATEGORY.equalsIgnoreCase(ele.getPayCat())) {
				...
				throw new USSDNonBlockingException(USSDExceptions.BPY00660.getBmgCode());
			    } else if (maxBillers <= benfSize) {
				ussdSessionMgmt.getTxSessions().put(USSDInputParamsEnum.PAY_BILLS_PAYEE_LIST.getTranId(),
					ele.getBnfLst().subList(0, maxBillers));
			    } else if (maxBillers >= benfSize) {
				ussdSessionMgmt.getTxSessions().put(USSDInputParamsEnum.PAY_BILLS_PAYEE_LIST.getTranId(), ele.getBnfLst());
			    }
